  ...
  useEffect(() => {
    const fetchFrames = async () => {
      try {
        ...
        const VinsFrames = data.frames?.filter(f => f.pro_brand?.toLowerCase() === 'vins') || [];
        setFrames(VinsFrames);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen w-full flex bg-gray-100 transition-all duration-300">
      ...
      <div className="flex-1 flex max-md:mt-20 flex-col items-center py-4 transition-all duration-300">
        ...
        <h1 className="text-2xl font-bold mb-4 text-gray-800">Vins-Optic Collection</h1>

        {loading ? (
          <div className="text-center text-gray-600 mt-10">Vins-Optic Collection Loading ...</div>
        ) : error ? (
