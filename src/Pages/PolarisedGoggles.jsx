    ...
    useEffect(() => {
        const fetchFrames = async () => {
            try {
                ...
                const polarisedgogglesFrames = data.frames?.filter(f => f.pro_material?.toLowerCase() === 'PolarisedGoggles') || [];
                setFrames(polarisedgogglesFrames);
            } catch (err) {
    ...
    return (
        <div className="min-h-screen w-full flex bg-gray-100 transition-all duration-300">
            ...
            <div className="flex-1 flex max-md:mt-20 flex-col items-center py-4 transition-all duration-300">
                ...
                <h1 className="text-2xl font-bold mb-4 text-gray-800">PolarisedGoggles Collection</h1>

                {loading ? (
                    <h4 className="text-center text-gray-600">PolarisedGoggles Collection Loading...</h4>
                ) : error ? (
...
export default PolarisedGoggles;
